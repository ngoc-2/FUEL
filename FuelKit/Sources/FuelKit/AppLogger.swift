...
public enum AppLogger {
    private static let subsystem = Bundle.main.bundleIdentifier ?? "com.stevennguyen.fuel"
...
extension AppLogger {
    ...
    public static func debug(_ message: String, category: Logger) {
        #if DEBUG
    ...
    public static func info(_ message: String, category: Logger) {
        category.info("\(message, privacy: .public)")
    }
    
    /// Log warning messages that don't prevent operation
    public static func warning(_ message: String, category: Logger) {
        category.warning("\(message, privacy: .public)")
    }
    
    /// Log error messages
    public static func error(_ message: String, error: Error? = nil, category: Logger) {
        if let error = error {
    ...
    public static func critical(_ message: String, error: Error? = nil, category: Logger) {
        if let error = error {
